...
This note records the initial PostgreSQL bring-up state for the TRAQ server and the steps needed to make the local database usable.
...
- ``pg_isready -h 127.0.0.1 -p 5432`` reports ``accepting connections`` when run outside the sandbox.
...
2. TCP as ``postgres``
   - ``psql -h 127.0.0.1 -U postgres`` reaches the server but requires a password.

This means PostgreSQL itself is running correctly. The remaining setup work is account/bootstrap configuration.
...
Artifacts such as PDFs, audio, and images should remain on disk or move to object storage later. PostgreSQL should become the system of record for metadata and workflow state.
