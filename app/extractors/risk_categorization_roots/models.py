"""Structured schema for the ``risk_categorization_roots`` extractor.
...
Purpose:
    ...
    registry for the ``risk_categorization_roots`` section.
...
class RiskCategorizationRootsExtraction(StrictBaseModel):
    section_id: Literal["risk_categorization_roots"] = Field(
        "risk_categorization_roots",
        description="Section identifier for this extraction.",
    )
    tree_part: Literal["Roots"] = Field(
        "Roots",
        description="Tree part for this risk categorization extraction.",
    )
    conditions: list[Matrix2Condition] = Field(
        default_factory=list,
        description="Ordered Matrix 2 rows for roots conditions of concern.",
    )
